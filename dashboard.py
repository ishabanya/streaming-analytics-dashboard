#!/usr/bin/env python3
"""
Streamlit Dashboard for the Streaming Platform ETL System
...
def get_recent_data(minutes: int = 30):
    ...
    # Get metrics
    metrics_df = db_manager.get_metrics_by_timeframe(start_time, end_time)
...
def display_key_metrics(logs_df: pd.DataFrame):
    ...
    with col1:
        ...
        <div class="metric-card">
            <div class="metric-value">{plays_per_min}</div>
...
def display_performance_metrics(logs_df: pd.DataFrame):
    ...
    if not logs_df.empty:
        ...
        metrics = metrics_calculator.calculate_performance_metrics(logs_df)
        ...
        with col2:
            ...
            perf_df = pd.DataFrame([
                ["Average Response Time", f"{metrics['avg_response_time']} ms"],
                ["Buffer Underrun Rate", f"{metrics['buffer_underrun_rate']}%"],
                ["Total Play Events", metrics['total_play_events']],
                ["Buffer Underruns", metrics['buffer_underruns']]
...
def main():
    ...
    # Get data
    logs_df, metrics_df = get_recent_data(minutes)
